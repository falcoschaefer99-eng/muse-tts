...
def scan_voices_dir():
    """Scan voices/ directory for reference WAVs."""
...
def muse_speak(text: str, voice: str = "", clone: str = "", ref_audio: str = "", speed: float = 0) -> str:
    ...
    Args:
        ...
        clone: Name of a voice clone (e.g. "pedro_pascal", "idris_elba"). Use muse_list_voices for options.
...
def muse_check() -> dict:
    ...
    if engine == "none" and clone_eng == "none":
        status["help"] = "Install: pip install muse-tts[macos] (Mac) or pip install muse-tts[cloning] (any platform)"
...
def main():
    """Entry point for the muse-tts CLI command."""
    engine = detect_engine()
    clone_eng = detect_clone_engine()
    engine_label = {
